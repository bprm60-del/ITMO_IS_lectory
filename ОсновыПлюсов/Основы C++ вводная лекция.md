...
```mermaid
flowchart TD
    Src["Исходный код (.cpp, .hpp)"] --> Prep["1. Препроцессинг (Preprocessor)<br/>Раскрытие #include, #define, замена макросов"]
    Prep --> Comp["2. Компиляция (Compiler)<br/>Синтаксический анализ (AST), семантика, оптимизация"]
    Comp --> Asm["3. Ассемблирование (Assembler)<br/>Трансляция ассемблера в машинный код"]
    Asm --> Obj["Объектные файлы (.o / .obj)<br/>Скомпилированный байткод с таблицей символов"]
    Obj --> Link["4. Линковка (Linker)<br/>Связывание символов, вызовов функций и библиотек (CRT, STL)"]
    Link --> Bin["Исполняемый бинарник (.exe / ELF)"]

    style Src fill:#e3f2fd,stroke:#1e88e5
    style Obj fill:#fff3e0,stroke:#fb8c00
    style Bin fill:#e8f5e9,stroke:#43a047,stroke-width:2px
```
...
graph TD
    Root["lab_work_N/"]
    Root --> CM["CMakeLists.txt (главный конфигурационный файл)"]
    Root --> Inc["include/ (публичные заголовочные файлы .hpp)"]
    Root --> Src["src/ (файлы реализации .cpp)"]
    Root --> Tests["tests/ (модульные тесты GoogleTest)"]
    
    style Root fill:#f5f5f5,stroke:#333
    style CM fill:#fff9c4,stroke:#fbc02d
